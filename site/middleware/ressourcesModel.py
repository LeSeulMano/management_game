...
def mange():
    ...
    if nourriture > (habitant * 10) :
        sql = "UPDATE stock SET quantite = " + str(nourriture - (habitant * 10)) + " WHERE (SELECT ressources.id_ressources FROM ressources WHERE ressources.name_res = 'Nourriture') = stock.id_ressources AND id_entrepot = (SELECT id_entrepot FROM entrepot, village WHERE entrepot.id_village = village.id_village AND village.id_user = 1);"
        execute(sql, {})
        return "Success"
    else: 
        tmp = nourriture // 10
        sql = "UPDATE stock SET quantite = " + str(nourriture - (tmp * 10)) + " WHERE (SELECT ressources.id_ressources FROM ressources WHERE ressources.name_res = 'Nourriture') = stock.id_ressources AND id_entrepot = (SELECT id_entrepot FROM entrepot, village WHERE entrepot.id_village = village.id_village AND village.id_user = 1);"
        execute(sql, {})
